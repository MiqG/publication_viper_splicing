...
run_cutpoint = function(df, surv_time_col, surv_event_col){
    result = tryCatch(
        {
            result_cut = surv_cutpoint(df, time=surv_time_col, event=surv_event_col, variables="feature_value")
            result_cat = surv_categorize(result_cut)
            fit_survdiff = survival::survdiff(
                as.formula(sprintf("Surv(%s, %s) ~ feature_value", surv_time_col, surv_event_col)), 
                data=result_cat
            )
