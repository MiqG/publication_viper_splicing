...
MAX_TARGETS = [100, 500, 1000, 2000, 5000]
...
rule all:
    input:
        ...
        expand(os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}",".done","done_pruning"), dataset=DATASETS, omic_type=OMIC_TYPES, max_targets=MAX_TARGETS),
        expand(os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned","network.txt"), dataset=DATASETS, omic_type=OMIC_TYPES, max_targets=MAX_TARGETS),
        expand(os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned","regulons.tsv.gz"), dataset=DATASETS, omic_type=OMIC_TYPES, max_targets=MAX_TARGETS),
        # make regulon set
        expand(os.path.join(RESULTS_DIR,"files","aracne_regulons_{dataset}_{max_targets}-{omic_type}"), omic_type=OMIC_TYPES, dataset=DATASETS, max_targets=MAX_TARGETS)
...
rule regulon_inference_aracne_prune_bootstraps:
    ...
    output:
        touch(os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}",".done","done_pruning"))
    params:
        boostrap_dir = os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}"),
        output_dir = os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}"),
        max_targets = lambda wildcards: int(wildcards.max_targets)
    threads: 1
    resources:
...
rule regulon_inference_aracne_java_consolidation:
    input:
        os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}",".done","done_threshold"),
        [os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}",".done","done_bootstrap_{boot_i}").format(dataset="{dataset}", omic_type="{omic_type}", boot_i=boot_i) for boot_i in BOOTSTRAPS],
        os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}",".done","done_pruning")
    output:
        os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned","network.txt")
    params:
        src = "scripts/aracne/ARACNe-AP",
        output_dir = os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned"),
...
rule prepare_regulons:
    input:
        regulators = lambda wildcards: GENEXPR_FILES[wildcards.dataset],
        targets = lambda wildcards: SPLICING_FILES[wildcards.dataset] if wildcards.omic_type!="genexpr" else GENEXPR_FILES[wildcards.dataset],
        aracne_network = os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned","network.txt")
    output:
        regulons = os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned","regulons.tsv.gz")
    threads: 1
    resources:
        runtime = 3600*1, # 1 h
        memory = 10 # GB
...
rule make_regulon_sets:
    input:
        regulons = os.path.join(RESULTS_DIR,"files","aracne_regulons","{dataset}-{omic_type}-{max_targets}","pruned","regulons.tsv.gz")
    output:
        regulons_dir = directory(os.path.join(RESULTS_DIR,"files","aracne_regulons_{dataset}_{max_targets}-{omic_type}"))
    run:
        import os
        import shutil
